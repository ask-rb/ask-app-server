...
Patch numbers advance by exactly one per release: `0.4.22` → `0.4.23` →
`0.4.24`. Never skip or jump patch numbers (`0.4.22` → `0.4.24` is wrong),
...
gemchain guard ask-app-server
gemchain update ask-app-server <new-version> --dry-run
gemchain update ask-app-server <new-version>
```
...
3. Build passes — `bundle exec rake build` (or `gem build ask-app-server.gemspec`).
4. Changelog — `Unreleased` entries moved under the new `X.Y.Z` heading with
   the release date.
5. Version — bumped in `lib/ask/app_server/version.rb` by gemchain, not by hand.
...
| `lib/ask/app_server/version.rb` at HEAD | `X.Y.Z` |
| Git tag | `vX.Y.Z` |
...
- Bug fix only: `0.4.22` → `0.4.23`.
- Several bug fixes ship together: one release, `0.4.22` → `0.4.23` (never
  `0.4.24`).
- New backwards-compatible API: `0.4.22` → `0.5.0`.
- Breaking change while pre-1.0: `0.5.0` → `0.6.0`, with a `Changed` /
  `Removed` changelog entry calling out the break.
- First stable release: `0.9.x` → `1.0.0`.
- This gem bumped because a dependency released: patch only, e.g. `0.4.23` →
  `0.4.24`, via the gemchain cascade — never hand-published.
